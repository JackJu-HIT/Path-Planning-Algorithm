%********根据参考文献基于改进A_算法和人工势场法智能小车路径规划研究_宣仁虎主程序**********%
clear all;
...
if jude1==jude2
    path_zhunbei=[path_zhunbei;[path1(i,1),path1(i,2)]];
else
  guaidian_x=[guaidian_x;path1(i,1)];
  guaidian_y=[guaidian_y;path1(i,2)];
  guaidian_x_start=[guaidian_x_start;path1(i-1,1)];
  guaidian_y_start=[guaidian_y_start;path1(i-1,2)];
end
end
% guaidian_x=sort(guaidian_x);
% guaidian_y=sort(guaidian_y);
guaidian=[guaidian_x,guaidian_y];
guaidian_start=[guaidian_x_start,guaidian_y_start];
% guaidian=[p.start;guaidian;p.goal]
%legend('A*算法轨迹');
% %**********基于基于改进A_算法和人工势场法智能小车路径规划研究_宣仁虎 人工势场法*******************……%
t1=clock;
%%%%test
%guaidian=[]
...
path_combine=[path_combine;path_zhunbei];
path_combine=unique(path_combine,'rows')
X1=[start(1);path_combine(:,1);goal(1)]%=[p.start(1);path_combine(:,1);p.goal(1)];
Y1=[start(1);path_combine(:,2);goal(2)]%[p.start(2);path_combine(:,2);p.goal(2)];
%计算时间
t2=clock;
art_time=etime(t2,t3)
m1=[X1];
N1=[Y1];
plot(m1,N1,'-g','LineWidth',2)%'LineWidth',2
legend('人工势场法');
hold on
% hold on
...
legend('Reference method [7]','Fusion algorithm proposed in this paper','Obstacles','Starting point','Terminal point');
hold on
%title("人工势场法和A*融合算法，加入50个障碍物仿真结果")
FontSize =25;
