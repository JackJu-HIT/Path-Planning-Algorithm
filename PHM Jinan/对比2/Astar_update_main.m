...
%%%%%改进前根据论文’基于改进A*算法的仓储物流移动机器人 任务调度和路径优化研究‘改进A*算法主程序A*算法************
 t1=clock;
[ path3 STEP_a_time ]=AStar_referance(obstacle,p,1);%A*算法
 t2=clock;
 TA_time=etime(t2,t1);
...
if N11>0
 ...
 [Goal  STEP_TIME]=deal_line_obstacle(obstacle,path_line_start_ago,path_line_goal_ago,p)
t10=clock;
 A_time=etime(t10,t9);
...
plot(p.start(1),p.start(2),'*r','LineWidth',3);hold on;
plot(p.goal(1),p.goal(2),'*b','LineWidth',3);hold on;
legend('Reference mthod[8]','Improved A-star algorithm','Obstacles','Starting point','Terminal point');
