function [path K close]=AStar_referance(obstacle,p,k)
path=[];%用于存储路径
%OpenList
open=[p.start(1) p.start(2) h(p.start,p.goal) 0 p.start(1) p.start(2)];%有问题！！！！！！！
%CloseList
close=[];
next=MotionModel(k);%更新状态--下一步的八个点
findFlag=false;%目标标志
K=1
M=0;
while ~findFlag
...
%         %%%%根据论文’改进A_算法的AGV路径规划_刘生伟‘引入新的启发函数
%        if abs(current(1,1)-p.goal(1))>abs(current(1,2)-p.goal(2))
%            m(3)=m(4)+abs((current(1,1)-p.goal(1)))*(h(m(1:2),p.goal)+h(current(1,5:6),p.goal));
%        else
%             m(3)=m(4)+abs((current(1,2)-p.goal(2))*(h(m(1:2),p.goal))+h(current(1,5:6),p.goal));
%        end
        if abs(current(1,1)-p.goal(1))>abs(current(1,2)-p.goal(2))
           m(3)=m(4)+2^(0.5)*abs(current(1,2)-p.goal(2))+abs(current(1,1)-p.goal(1))+abs(current(1,2)-p.goal(2))-2*abs(current(1,2)-p.goal(2));
       else
            m(3)=m(4)+2^(0.5)*abs(current(1,1)-p.goal(1))+abs(current(1,1)-p.goal(1))+abs(current(1,2)-p.goal(2))-2*abs(current(1,1)-p.goal(1));
       end
