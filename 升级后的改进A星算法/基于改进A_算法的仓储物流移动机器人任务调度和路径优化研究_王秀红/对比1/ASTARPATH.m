...
RegisteredGoals=[row col];%%目标坐标索引
    ...
    for p=1:N_Neighboors
        ...
        if(ClosedMAT(CurrentY+i,CurrentX+j)==0) %Neiboor is open;
            ...
            if Flag==1;           
                ...
                %FScore(CurrentY+i,CurrentX+j)= tentative_gScore+1.5*Hn(CurrentY+i,CurrentX+j);
                if abs(CurrentX+j-RegisteredGoals(2))>abs(CurrentY+i-RegisteredGoals(1))
                    FScore(CurrentY+i,CurrentX+j)=tentative_gScore+2^(0.5)*abs(CurrentY+i-RegisteredGoals(1))+abs(CurrentX+j-RegisteredGoals(2))+abs(CurrentY+i-RegisteredGoals(1))-2*abs(CurrentY+i-RegisteredGoals(1));
                else
                    FScore(CurrentY+i,CurrentX+j)=tentative_gScore+2^(0.5)*abs(CurrentX+j-RegisteredGoals(2))+abs(CurrentX+j-RegisteredGoals(2))+abs(CurrentY+i-RegisteredGoals(1))-2*abs(CurrentX+j-RegisteredGoals(2));
                end
            end
        end
    end
